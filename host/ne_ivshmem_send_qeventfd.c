 ...
 *  desc : sends periodic irqs to a QEMU ivshmem device via an eventfd(2) file descriptor.
 *  
...
static int process_server_msg(UNIXSockData * s, const uint8_t * buf)
{
	int msg_fd, tmp_fd;
	long msg_id;
	...
	prdbg("Processing msgID: %ld, flag: %d\n", msg_id, tmp_fd);
...
int main(int argc, char **argv)
{
	...
	while (1) {
		int c;
		if ((c =
		     getopt_long(argc, argv, "hf:", long_option, NULL)) < 0)
			break;
		switch (c) {
