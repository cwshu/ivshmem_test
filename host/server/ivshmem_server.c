...
int main(int argc, char **argv)
{
	...
	if (ftruncate(s->shm_fd, s->shm_size) != 0) {
		fprintf(stderr,
			"ivshmem server: could not truncate memory region\n");
		exit(-1);
	}
	...
	for (;;) {
		...
		if (handle == s->conn_socket) {
			...
			s->maxfd =
			    ...
			    s->maxfd ? s->live_vms[s->total_count -
						   1].sockfd : s->maxfd;
			...
			for (j = 0; j < s->total_count; j++) {
				...
				if (s->live_vms[j].alive) {
					printf
					    ("[UD] sending kill of fd[%ld] to %ld\n",
					     deadposn, j);
					sendKill(s->live_vms[j].sockfd,
						 deadposn, sizeof(deadposn));
				}
			}
